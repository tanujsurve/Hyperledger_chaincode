...
installChaincodes() {
  if [ -n "$(ls "$CHAINCODES_BASE_DIR/./chaincodes/chaincode-golang")" ]; then
    local version="0.0.1"
    printHeadline "Packaging chaincode 'chaincode1'" "U1F60E"
    chaincodeBuild "chaincode1" "golang" "$CHAINCODES_BASE_DIR/./chaincodes/chaincode-golang" "16"
    chaincodePackage "cli.org1.example.com" "peer0.org1.example.com:7041" "chaincode1" "$version" "golang" printHeadline "Installing 'chaincode1' for Org1" "U1F60E"
    ...
    echo "Looked in dir: '$CHAINCODES_BASE_DIR/./chaincodes/chaincode-golang'"
  fi
