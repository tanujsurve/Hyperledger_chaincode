...
channelQuery() {
  ...
  then
    ...
    peerChannelGetInfo "my-channel2" "cli.org2.example.com" "peer1.org2.example.com:7062"

  elif [ "$1" = "fetch" ] && [ "$2" = "config" ] && [ "$3" = "my-channel2" ] && [ "$4" = "org2" ] && [ "$5" = "peer1" ]; then
    TARGET_FILE=${6:-"$channel-config.json"}

    peerChannelFetchConfig "my-channel2" "cli.org2.example.com" "$TARGET_FILE" "peer1.org2.example.com:7062"

  elif [ "$1" = "fetch" ] && [ "$3" = "my-channel2" ] && [ "$4" = "org2" ] && [ "$5" = "peer1" ]; then
    BLOCK_NAME=$2
    TARGET_FILE=${6:-"$BLOCK_NAME.block"}

    peerChannelFetchBlock "my-channel2" "cli.org2.example.com" "${BLOCK_NAME}" "peer1.org2.example.com:7062" "$TARGET_FILE"

  elif
    [ "$1" = "getinfo" ] && [ "$2" = "my-channel2" ] && [ "$3" = "org3" ] && [ "$4" = "peer1" ]
  then

    peerChannelGetInfo "my-channel2" "cli.org3.example.com" "peer1.org3.example.com:7082"

  elif [ "$1" = "fetch" ] && [ "$2" = "config" ] && [ "$3" = "my-channel2" ] && [ "$4" = "org3" ] && [ "$5" = "peer1" ]; then
    TARGET_FILE=${6:-"$channel-config.json"}

    peerChannelFetchConfig "my-channel2" "cli.org3.example.com" "$TARGET_FILE" "peer1.org3.example.com:7082"

  elif [ "$1" = "fetch" ] && [ "$3" = "my-channel2" ] && [ "$4" = "org3" ] && [ "$5" = "peer1" ]; then
    BLOCK_NAME=$2
    TARGET_FILE=${6:-"$BLOCK_NAME.block"}

    peerChannelFetchBlock "my-channel2" "cli.org3.example.com" "${BLOCK_NAME}" "peer1.org3.example.com:7082" "$TARGET_FILE"
...
printChannelsHelp() {
  ...
  echo "fablo channel getinfo my-channel2 org3 peer1"
  echo -e "\t Get channel info on 'peer1' of 'Org3'".
  echo ""
  echo "fablo channel fetch config my-channel2 org3 peer1 [file-name.json]"
  echo -e "\t Download latest config block and save it. Uses first peer 'peer1' of 'Org3'".
  echo ""
  echo "fablo channel fetch <newest|oldest|block-number> my-channel2 org3 peer1 [file name]"
